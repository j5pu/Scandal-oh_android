...
public class CommentsActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent() != null){
			scandaloh_owner = getIntent().getExtras().getString(ScandalohFragment.SOURCE_NAME);
		...
		commentsAdapter = new CommentAdapter(this,R.layout.comment_izquierda, R.layout.comment_derecha, array_comments, scandaloh_owner);
	...
	public void onStart(){
		...
		if (MyApplication.logged_user){
			edit_comment.setText(getResources().getString(R.string.escribe_un_comentario));
		...
		// Si no está logueado le mandamos a la pantalla de login
			...
			edit_comment.setText(getResources().getString(R.string.inicia_sesion_para_comentar));
			edit_comment.setOnClickListener(new View.OnClickListener() {
				
				@Override
				public void onClick(View v) {
					Intent i = new Intent(CommentsActivity.this, LoginSelectActivity.class);
					startActivity(i);
				}
			});
		}
		...
		protected void onPostExecute(Integer result) {
			...
			if (any_error) {
				...
				if (result >= 200 && result < 300) {
					...
					Toast toast;
					toast = Toast.makeText(mContext,getResources().getString(
							R.string.hubo_algun_error_enviando_comentario),Toast.LENGTH_LONG);
					toast.show();
					// Quitamos el ProgressDialog
					if (progress.isShowing()) {
						progress.dismiss();
